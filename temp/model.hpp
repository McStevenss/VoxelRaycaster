...
public:
    Model(string const &path, bool gamma = false, bool flip_uvs = true) : gammaCorrection(gamma)
    {
        mPath = path;
    ...
    Model(string const &path, bool gamma = false, bool flip_uvs = true, bool has_animation=false) : gammaCorrection(gamma)
    {
        mPath = path;
        loadModel(path,flip_uvs);
...
private:
    ...
    void loadModel(string const &path, bool flip_uvs = true)
    {
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        for(unsigned int i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            // aabbMin =  glm::min(aabbMin, aabbMin);
            // aabbMax =  glm::max(aabbMax, aabbMax);
